...
GrandSlam data set splited by 20% test and 80% training by random selection. 
...
# Train the model
model <- train(RESULT ~ ., data = GrandSlam_train, method = "svmLinear", trControl = train_cntrl)
...
weights <- colSums(final_model@coef[[1]] * final_model@xmatrix[[1]])
...
pretict_grandslam <- predict(model,GrandSlam_test)
confusionMatrix(pretict_grandslam, GrandSlam_test$RESULT)
